...
$scope.sendTrans = function(){
  ...
  var fromAscii2 = function(str, padding) {
    var hex = '0x';
  ...
  var toAmount = parseFloat($('#amount2').val());

 // alert($('#amount2').val());
 // alert($('#amount').val());
 try{
  lastDataFromTrans = fromAscii2($('#comment').val());
}
catch(err){
 // alert(err);
}
  //alert(lastDataFromTrans)
...
$scope.unsupport = true;

$scope.toggleKey = function(){


  $('#theKey').attr('type', 'text');


  $('#theKey').on('blur', function(){
      setTimeout(function(){
        $('#theKey').attr('type', 'password');
